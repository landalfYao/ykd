<template>
    <div>
        <h2><el-button @click="navTo2('/goods_cate',formData.shopid)" size="small" icon="el-icon-arrow-left" circle ></el-button> 分类编辑</h2>
        ...
        <div class="panel-start">
            ...
            <div style="width:400px;padding-top:20px">
                <Form :model="formData" label-position="top">
                    ...
                    <FormItem label="分类名称">
                        <Input v-model="formData.name" placeholder="分类名称" style="width:400px" />
                    </FormItem>
                    <FormItem label="提交">
...
export default {
  ...
  methods: {
    ...
    submitTichtext() {
      ...
      this.$http
        .post(this.com.JAVA_API + "category/" + url, this.formData,{ emulateJSON:true })
        .then(res => {
          that.loading = false;
          if (res.data.code) {
            that.$message({
            ...
            that.$router.push({path:'/goods_cate',query:{id:that.$route.query.id}})
          } else {
            that.$message({
              showClose: true,
              message: res.data.msg,
